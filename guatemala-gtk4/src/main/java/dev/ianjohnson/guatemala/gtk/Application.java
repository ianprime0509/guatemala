...
public class Application extends dev.ianjohnson.guatemala.gio.Application {
    ...
    public static Application of(String applicationId, Set<ApplicationFlag> flags) {
        MemoryAddress memoryAddress = BindingSupport.callThrowing(local ->
                (MemoryAddress) GTK_APPLICATION_NEW.invoke(local.allocateUtf8String(applicationId), Flag.toInt(flags)));
        return new Application(memoryAddress);
    }

    public Optional<Window> getActiveWindow() {
        MemoryAddress addr = BindingSupport.callThrowing(
                () -> (MemoryAddress) GTK_APPLICATION_GET_ACTIVE_WINDOW.invoke(getMemoryAddress()));
        return MemoryAddress.NULL.equals(addr) ? Optional.empty() : Optional.of(Window.TYPE.wrapInstance(addr));
    }
